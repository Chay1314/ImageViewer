...
LoadImageThread::LoadImageThread(const wxString& file_path, wxImage* image, wxFrame* frame)
    : file_path_(file_path), image_(image), image_frame_(frame) {
}
...
void* LoadImageThread::Entry() {
  wxImage image("E:\\file\\picure\\thumbnail.bmp");
  wxBitmap bitmap = image;
  int xpos = (image_frame_->GetClientSize().x - image.GetWidth()) / 2;
  int ypos = (image_frame_->GetClientSize().y - image.GetHeight()) / 2;

  wxStaticBitmap* static_bitmap = new wxStaticBitmap(image_frame_, wxID_ANY, bitmap, 
                                                     wxPoint(xpos, ypos), wxDefaultSize);
