...
void DnDFile::SetImage(const wxString& path_name) {
  ...
  wxFrame* frame = new wxFrame(top_window, wxID_ANY, _T(""), wxDefaultPosition, wxDefaultSize,
                               wxCAPTION | wxRESIZE_BORDER | wxCLOSE_BOX);
  frame->Show(true);

  ImageWindow* image_window_ = new ImageWindow(frame, wxID_ANY, wxDefaultPosition,
                                               frame->GetClientSize(), image);
